...
扩展 `obsidian-knowledge.ts` 提供 7 个工具：obs-ingest, obs-query, obs-compile, obs-weave, obs-lint, obs-capture, obs-reference。
知识库位于 `/mnt/d/DB/Obsidian/LLM-Wiki/`（已启用 Git 版本控制）。

**核心规则**（不遵守会导致知识流失或库内矛盾）：
- 会话结束 → `obs-ingest` 复盘（agent_end 自动兜底）
- 积累 ≥5 篇 → `obs-compile` 逐篇编译
- 编译后 → `obs-weave` 织入已有页面（不可跳过）
- 编译后 → `obs-lint` 健康检查
- 查询中获得的关键信息 → `obs-capture` 回流 wiki
- compile 前必须向用户展示关键发现并确认重点
